...
- The first packet smoke is now complete on the same 4-frame surface.
...
- A heavy delta-dominant concat sweep (`weight=8.0`) also reaches `top1_accuracy=0.75`, which means the current signal is dominated by temporal change rather than static packet semantics.
- The correct next move is no longer "generic feature packet"; it is a delta-dominant packet route on the same frozen upstream checkpoint, sample ids, and 4-frame tiny-local surface.
...
Next durable action:
- record the first packet smoke as a durable route decision
