...
# Launch one bounded shared-gating repair run that keeps the same frozen widened
# surface, semantic-change weighting, and temporal-delta consistency, but swaps
# relation supervision for a predicted-feature variance-floor repair.
...
EXP_NAME="${EXP_NAME:-shared_gating_pred_variance_repair_ego4d16f_smoke_r1}"
...
echo "Launching bounded shared-gating predicted-variance repair smoke"
...
python "${NVRC_ROOT}/main_nvrc.py" \
  ...
  --train-task-config "${NVRC_ROOT}/scripts/configs/tasks/overfit/l1_teacher-resnet18-variance-semchange-delta.yaml" \
  --eval-task-config "${NVRC_ROOT}/scripts/configs/tasks/overfit/l1_teacher-resnet18-variance-semchange-delta.yaml" \
