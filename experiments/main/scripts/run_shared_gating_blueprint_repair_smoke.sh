...
# feature target to the target-conditioned semantic blueprint while preserving
...
EXP_NAME="${EXP_NAME:-shared_gating_blueprint_repair_ego4d16f_smoke_r1}"
...
echo "Launching bounded shared-gating blueprint repair smoke"
...
python "${NVRC_ROOT}/main_nvrc.py" \
  ...
  --train-task-config "${NVRC_ROOT}/scripts/configs/tasks/overfit/l1_teacher-resnet18-blueprint-semchange-delta.yaml" \
  --eval-task-config "${NVRC_ROOT}/scripts/configs/tasks/overfit/l1_teacher-resnet18-blueprint-semchange-delta.yaml" \
