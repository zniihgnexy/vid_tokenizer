...
def vidx_to_pidx(vidx, vidx_max, pidx_max):
    ...
    pidx_t, pidx_h, pidx_w = [scales[d] * vidx[:, d][:, None] + \
                              torch.arange(scales[d], device=vidx.device)[None, :] for d in range(3)]
...
def video_to_patch(video, patch_size):
    ...
    patch = video.view(-1, n_patch_t, patch_size[0],
                       n_patch_h, patch_size[1], n_patch_w, patch_size[2], video.shape[-1]) \
                 .permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous() \
                 .view(-1, patch_size[0], patch_size[1], patch_size[2], video.shape[-1])
    return patch
...
def patch_to_video(patch, video_size):
    ...
    video = patch.view(-1, n_patch_t, n_patch_h, n_patch_w,
                       patch.shape[1], patch.shape[2], patch.shape[3], patch.shape[-1]) \
                 .permute(0, 1, 4, 2, 5, 3, 6, 7).contiguous() \
                 .view(-1, video_size[0], video_size[1], video_size[2], patch.shape[-1])
    return video
...
def _compute_pixel_idx_nd(n, idx, idx_max, sizes, padding, clipped=True, return_mask=True):
    ...
    px_idx = [idx[:, d][:, None] * patch_sizes[d] - padding[d] + \
              torch.arange(patch_sizes_padded[d], device=idx.device)[None, :] for d in range(n)]
...
def compute_paddings(output_patchsize=(1, 120, 120), scales=((1, 5, 5), (1, 4, 4), (1, 3, 3), (1, 2, 2)),
    ...
    This is just an approxiamate to the padding required. The exact way is to calculate the boundary coordinates of the patches.
...
def compute_paddings_v2(scales=((1, 5, 5), (1, 4, 4), (1, 3, 3), (1, 2, 2)),
                        kernels=((0, 1, 1), (0, 1, 1), (0, 1, 1), (0, 1, 1)), 
                        depths=(3, 3, 3, 1), resize_methods='trilinear'):
    """
    Compute the required padding sizes.
    This is just an approxiamate to the padding required. The exact way is to calculate the boundary coordinates of the patches.
    Length of returned padding == len(scales) + 1 == len(padding_per_layer) + 1 == len(depths) + 1
    """
    assert len(scales) == len(depths) == len(kernels)    
    paddings = np.zeros(3, dtype=np.int32)
    scales = np.array(scales)
    kernels = np.array(kernels).clip(min=1)
    paddings_reversed = []
    for i in reversed(range(len(scales))):
        assert np.all((kernels[i] - 1) % 2 == 0)
        assert np.all(scales[i] >= 1.), 'scales must be positive'
        paddings += depths[i] * (kernels[i] - 1) // 2
        if resize_methods == 'trilinear':
            paddings_reversed.append(paddings.tolist())
            paddings = np.floor(paddings / scales[i]).astype(np.int32) + (scales[i] > 1.)
        elif resize_methods == 'nearest':
            paddings_reversed.append(paddings.tolist())
            paddings = np.floor(paddings / scales[i]).astype(np.int32)
        else:
            raise NotImplementedError
    paddings_reversed.append(paddings.tolist())
    return list(reversed(paddings_reversed))